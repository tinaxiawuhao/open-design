...
import { deepseekHarnessAgentDef, parseModels } from '../../src/runtimes/defs/deepseek-harness.js';
...
describe('DeepSeek Harness profile frame validation', () => {
  test('declares stream-captured session resume instead of CLI-argument resume', () => {
    assert.equal(deepseekHarnessAgentDef.capturesSessionIdFromStream, true);
  ...
  test('maps the Harness catalog into provider-qualified model choices', () => {
    assert.deepEqual(parseModels(JSON.stringify({
      v: 1,
      type: 'models',
      runtime: 'open-design',
      models: [{
